# iAngel Moteur + API Intelligente Fusionnés
...
# Chargement des modules ML / JSON dans le moteur
app = Flask(__name__)
engine = INSFLocalLearner()
...
reassurance_bank = load_json("reassurance_bank.json")
humour_bank = load_json("humour_bank.json")
cyber_bank = load_json("cybersecurity_bank.json")
knowledge_base = load_json("insf_knowledge_base.json")
scan_model = load_pkl("scan_phone_model_v2.pkl")
anxiety_model = load_pkl("dummy_anxiety_voice_model.pkl")
anxiety_scaler = load_pkl("dummy_anxiety_feature_scaler.pkl")

@app.route("/")
def index():
    return jsonify({
        "status": "iAngel Fusionné avec Mémoire, API et Intelligence Émotionnelle",
        "users": list(engine._users_data.keys())
    })

@app.route("/tts")
def tts():
    text = request.args.get("text", "Bonjour, ici iAngel.")
    tts = gTTS(text, lang="fr")
    tts.save("sol_voice.mp3")
    return send_file("sol_voice.mp3", mimetype="audio/mpeg")

@app.route("/get_reassurance")
def get_reassurance():
    return jsonify({"message": reassurance_bank[0] if reassurance_bank else "Tout va bien se passer."})

@app.route("/get_joke")
def get_joke():
    return jsonify({"humour": humour_bank[0] if humour_bank else "Tu veux rire ? Mon disque dur aussi."})

@app.route("/get_cyber_tip")
def get_cyber_tip():
    return jsonify({"tip": cyber_bank[0] if cyber_bank else "Active la double authentification."})

@app.route("/get_fact")
def get_fact():
    return jsonify({"fact": knowledge_base[0] if knowledge_base else "Savais-tu que la cybersécurité commence par la prudence ?"})
...
def scan_phone():
    ...
    if scan_model:
        prediction = scan_model.predict([features])[0]
        return jsonify({"scan_result": prediction})
    return jsonify({"error": "Modèle de scan non disponible."})

@app.route("/analyze_voice", methods=["POST"])
def analyze_voice():
    data = request.get_json()
    features = data.get("features", [])
    if anxiety_model and anxiety_scaler:
        scaled = anxiety_scaler.transform([features])
        prediction = anxiety_model.predict(scaled)[0]
        return jsonify({"anxiety_level": prediction})
    return jsonify({"error": "Modèle d'anxiété non disponible."})
